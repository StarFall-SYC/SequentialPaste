# 顺序粘贴工具(Sequential Paste Tool) v1.0
...
### 3. 运行程序
```bash
python sequential_paste_gui.py
```
...
### v1.1.0 (2026-01-15)
*   🖥️ **多屏支持**：完美支持多显示器环境，悬浮窗边缘检测算法升级。
*   💾 **数据持久化**：自动保存进度、文件路径及悬浮窗位置至系统标准配置目录。
*   🐧 **全平台兼容**：深度优化 macOS 和 Linux 的开机自启与快捷键体验。
*   📦 **单文件运行**：图标资源内嵌，无需依赖外部图片文件。
*   💡 **启动提示**：若悬浮窗启动时位于边缘，将短暂显示位置提示。

### v1.0.0 (2026-01-15)
*   🎉 **正式发布**：核心功能趋于稳定。
*   ✨ **交互优化**：悬浮窗增加锁定功能，唤出逻辑优化（垂直范围检测）。
*   🔧 **修复**：修复快捷键冲突问题，将回退键调整为 `Alt+Ctrl+V`。
*   🎨 **UI升级**：全新的列表视图，支持双击跳转；统一的按钮风格。
